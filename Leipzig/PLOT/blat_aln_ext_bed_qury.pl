#!/usr/bin/perl 
### Script to extract gap regions from a psl file
...
$outfile =~ s/\.psl*//;
my $outfile1 = $outfile."_query_aln.bed";
my $outfile2 = $outfile."_target_aln.bed";
...
while( my $theline = <$info>)  {   
	...
	for (my $count=0; $count < $blockcount; $count++) {
		my $blocknumber = $count+1;
		if ($thisline[8] eq '+') {
#		if ($thisline[8] eq '+')	{
#10,11,12			14,15,16
			my $count2=$count+1;
			my $QueryStart = $qStarts[$count];
			my $QueryEnd = $qStarts[$count] + $blocksizes[$count];
                        my $AlignsizeQ = $QueryEnd - $QueryStart;
			my $TargetStart = $tStarts[$count];
                        my $TargetEnd = $tStarts[$count] + $blocksizes[$count];
                        my $AlignsizeT = $TargetEnd - $TargetStart;

			print $outQuery $thisline[9],"\t",$QueryStart,"\t",$QueryEnd,"\t",$thisline[10],"\t",$thisline[9],"+",$blocknumber,":",$AlignsizeQ,"\t+\t",$thisline[13],"\t",$TargetStart,"\t",$TargetEnd,"\t",$thisline[14],"\t",$linenumber,"_#_",$Line_count,"\t",$AlignsizeQ,"\t",$AlignsizeT,"\n";
##			print $outTarget $thisline[13],"\t",$TargetStart,"\t",$TargetEnd,"\t",$thisline[14],"\t",$thisline[9],"\t+\t",$thisline[13],"+",$blocknumber,":",$AlignsizeT,"\t",$thisline[9],":",$AlignsizeQ,"\t",$AlignsizeT,"\t",$AlignsizeQ,"\n";
			#Tname	Tstart	Tend	Tsize	Qname	Strand	Block:Talignsize	Qname:Qalignsize	Strand	Talignsize	Qalignsize
			}
