...
#		if ($thisline[8] eq '+')	{
			...
			my $InsertsizeQ = $QueryEnd - $QueryStart;		# Here it is alignment size
                        my $TargetEnd = $tStarts[$count2];
                        my $TargetStart = $tStarts[$count] + $blocksizes[$count];	# Here it is alignment size
			my $InsertsizeT = $TargetEnd - $TargetStart;
			print $outQuery $thisline[9],"\t",$QueryStart,"\t",$QueryEnd,"\t",$thisline[10],"\t",$thisline[9],"+",$blocknumber,":",$InsertsizeQ,"\t+\t",$thisline[13],"\t",$TargetStart,"\t",$TargetEnd,"\t",$thisline[14],"\t",$linenumber,"_#_",$Line_count,"\t",$InsertsizeQ,"\t",$InsertsizeT,"\n";
			##print $outTarget $thisline[13],"\t",$TargetStart,"\t",$TargetEnd,"\t",$thisline[14],"\t",$thisline[9],"\t+\t",$thisline[14],"+",$blocknumber,":",$InsertsizeT,"\t",$thisline[9],":",$InsertsizeQ,"\t",$InsertsizeT,"\t",$InsertsizeQ,"\n";
			}
			...
			my $QueryEnd = $qStarts[$count2] - $blocksizes[$count];
			my $InsertsizeQ = $QueryEnd - $QueryStart;		# Here it is alignment size	
                        my $TargetEnd = $tStarts[$count2];
                        my $TargetStart = $tStarts[$count] + $blocksizes[$count] - 1;	# Here it is alignment size
			my $InsertsizeT = $TargetEnd - $TargetStart;
			print $outQuery $thisline[9],"\t",$QueryStart,"\t",$QueryEnd,"\t",$thisline[10],"\t",$thisline[9],"-",$blocknumber,":",$InsertsizeQ,"\t-\t",$thisline[13],"\t",$TargetStart,"\t",$TargetEnd,"\t",$thisline[14],"\t",$linenumber,"_#_",$Line_count,"\t",$InsertsizeQ,"\t",$InsertsizeT,"\n";
