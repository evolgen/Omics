...
#biocLite("DESeq2")
#biocLite(c("LSD","gplots","pheatmap","PoiClaClu","EDASeq","DESeq"))
library(geneplotter)
library(ggplot2)
library(plyr)
library(LSD)
...
setwd("/scr/k70san/rohit/Lacertidae/splicing/pooled/ALL/HTseq")
# Load directory and get mapping-files

#### Working version
directory <- file.path("/scr/k70san/rohit/Lacertidae/splicing/pooled/ALL/HTseq")
sampleFiles <- list.files(path="/scr/k70san/rohit/Lacertidae/splicing/pooled/ALL/HTseq",pattern="^new_features_")


### For Condition and Group
sampleGroup <- factor(rep.int(c("Bilineata","Viridis"),5))
sampleCondition = c(rep.int(c(rep("Brain",2), rep("Gonads",2),rep("Heart",2), rep("Kidney",2),rep("Liver",2)),1))
...
res <- data.frame(assay(rld), avgLogExpr = ( assay(rld)[,2] + assay(rld)[,1] ) / 2,
...
png(file="~/Downloads/scripts/differential_expression/Paper_deseq2_PCA.png",width=1200,height=1100)
...
idx <- rowSums( counts(dds, normalized=TRUE) >= 15 ) >= 3
...
dev.copy(png,'~/Downloads/scripts/differential_expression/Paper_deseq2_multidensity_mod.png')
dev.off()
multiecdf( counts(estimateSizeFactors(ddsHTSeq), normalized = T)[idx.nz ,],
           xlab="mean counts", xlim=c(0, 1000))
dev.copy(png,'~/Downloads/scripts/differential_expression/Paper_deseq2_multiecdf_mod.png')
dev.off()

# png(file="~/Downloads/scripts/differential_expression/Paper_deseq2_MDplot_pairwise.png",width=1200,height=1100)
# pdf("~/Downloads/scripts/differential_expression/Paper_deseq2_MDplot_pairwise.pdf")
# MA.idx = t(combn(1:10, 2))
# for( i in  seq_along( MA.idx[,1])){ 
#   glMDPlot(counts(estimateSizeFactors(ddsHTSeq), normalized = T)[idx.nz ,], 
...
png(file="~/Downloads/scripts/differential_expression/Paper_deseq2_dispest.png",width=1200,height=1100)
...
dev.copy(png,'~/Downloads/scripts/differential_expression/Paper_deseq2_MAplot_mod.png')
dev.off()

resLFC1 <- results(dds, lfcThreshold=1)
topGene <- rownames(resLFC1)[which.max(resLFC1$log2FoldChange)]
...
table(res_2$padj < 0.01)
# FALSE  TRUE 
# 18884   402 


png(file="~/Downloads/scripts/differential_expression/Paper_deseq2_filterrej.png",width=1200,height=1100)
par(mfrow=c(1,2))
...
png(file="~/Downloads/scripts/differential_expression/Paper_deseq2_pvaluecheck.png",width=1200,height=1100)
...
png(file="~/Downloads/scripts/differential_expression/Paper_deseq2_pvaluecorrected.png",width=1200,height=1100)
...
write.table(diff_genes_1_1, "/scr/k70san/rohit/Lacertidae/splicing/pooled/ALL/HTseq/paper_differentially_expressed.txt", sep="\t")

diff_genes_1_2 <- res[which(res$padj < 0.05 & res$log2FoldChange < 0),]
write.table(diff_genes_1_2, "/scr/k70san/rohit/Lacertidae/splicing/pooled/ALL/HTseq/paper_lowly_expressed.txt", sep="\t")


diff_genes_2_1 <- res_2[which(res_2$padj < 0.05 & res_2$log2FoldChange  > 1),]
write.table(diff_genes_2_1, "/scr/k70san/rohit/Lacertidae/splicing/pooled/ALL/HTseq/paper_differentially_expressed_2.txt", sep="\t")

diff_genes_2_2 <- res_2[which(res_2$padj < 0.05 & res_2$log2FoldChange < 0),]
write.table(diff_genes_2_2, "/scr/k70san/rohit/Lacertidae/splicing/pooled/ALL/HTseq/paper_lowly_expressed_2.txt", sep="\t")


ddsLRT <- DESeq(dds, test="LRT", reduced= ~ 1)
resLRT <- results(ddsLRT)
summary(resLRT)
# out of 18918 with nonzero total read count
# adjusted p-value < 0.1
# LFC > 0 (up)     : 201, 1.1% 
# LFC < 0 (down)   : 517, 2.7% 
# outliers [1]     : 2477, 13% 
...
resLRT <- resLRT[order(resLRT$log2FoldChange),]
resLRT <- resLRT[ !is.na(resLRT$padj), ]
resLRT <- resLRT[ !is.na(resLRT$pvalue), ]
head(resLRT)

DESeq2::plotMA(ddsLRT,ylim=c(-11,11),main='DESeq2 with Groups - LRT')
dev.copy(png,'~/Downloads/scripts/differential_expression/Paper_deseq2_MAplot_mod_LRT.png')
dev.off()

diff_genes_LRT <- resLRT[which(resLRT$padj < 0.01 & resLRT$log2FoldChange > 1),]
write.table(diff_genes_LRT, "/scr/k70san/rohit/Lacertidae/splicing/pooled/ALL/HTseq/paper_differentially_expressed_LRT.txt", sep="\t")

diff_genes_2_LRT <- resLRT[which(resLRT$padj < 0.01 & resLRT$log2FoldChange < 0),]
write.table(diff_genes_2_LRT, "/scr/k70san/rohit/Lacertidae/splicing/pooled/ALL/HTseq/paper_lowly_expressed_LRT.txt", sep="\t")
...
dev.copy(png,'~/Downloads/scripts/differential_expression/Paper_deseq2_MAplot_group.png')
dev.off()
...
dev.copy(png,'~/Downloads/scripts/differential_expression/Paper_deseq2_MAplot_cond.png')
dev.off()
...
png(file="~/Downloads/scripts/differential_expression/Paper_deseq2_MAplot_cond_pairwise.png",width=1100,height=800)
par(mfrow=c(5,2))
...
#dev.copy(png,'~/Downloads/scripts/Paper_deseq2_MAplot_cond_pairwise.png')
dev.off()
...
data <- plotPCA(rld, intgroup = c("group","condition"), returnData=TRUE)
percentVar <- round(100 * attr(data, "percentVar"))
png(file="~/Downloads/scripts/differential_expression/Paper_deseq2_PCA_int.png",width=1200,height=1100)
ggplot(data, aes(PC1, PC2, color=group.1, shape=condition)) + geom_point(size=3) + 
  xlab(paste0("PC1: ",percentVar[1],"% variance")) +
  ylab(paste0("PC2: ",percentVar[2],"% variance")) + coord_fixed()
dev.off()
...
multidensity( counts(estimateSizeFactors(ddsHTSeq), normalized = T)[idx.nz ,],
              xlab="mean counts", xlim=c(0, 1000))
dev.copy(png,'~/Downloads/scripts/differential_expression/Paper_deseq2_multidensity_mod_int.png')
dev.off()
multiecdf( counts(estimateSizeFactors(ddsHTSeq), normalized = T)[idx.nz ,],
           xlab="mean counts", xlim=c(0, 1000))
dev.copy(png,'~/Downloads/scripts/differential_expression/Paper_deseq2_multiecdf_mod_int.png')
dev.off()

#png(file="~/Downloads/scripts/differential_expression/Paper_deseq2_MDplot_pairwise_int.png",width=1200,height=1100)
pdf("~/Downloads/scripts/differential_expression/Paper_deseq2_MDplot_pairwise_int.pdf")
...
png(file="~/Downloads/scripts/differential_expression/Paper_deseq2_dispest_int.png",width=1200,height=1100)
plotDispEsts(estimateDispersions(estimateSizeFactors(ddsHTSeq)))
dev.off()
...
dev.copy(png,'~/Downloads/scripts/differential_expression/Paper_deseq2_MAplot_mod_int.png')
dev.off()
...
png(file="~/Downloads/scripts/differential_expression/Paper_deseq2_filterrej_int.png",width=1200,height=1100)
par(mfrow=c(1,2))
plot(metadata(res_2)$filterNumRej, type="b", xlab="quantiles of 'baseMean'",
     ylab="number of rejections")
hist(res_2$pvalue, col = "lavender", main = "WT vs Deletion", xlab = "p-values")
...
png(file="~/Downloads/scripts/differential_expression/Paper_deseq2_pvaluecheck_int.png",width=1200,height=1100)
...
png(file="~/Downloads/scripts/differential_expression/Paper_deseq2_pvaluecorrected_int.png",width=1200,height=1100)
...
dev.copy(png,'~/Downloads/scripts/differential_expression/Paper_deseq2_MAplot_mod_LRT_int.png')
dev.off()
