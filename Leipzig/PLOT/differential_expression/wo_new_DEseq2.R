...
directory <- file.path("/scr/k70san/rohit/Lacertidae/splicing/pooled/namesort/counts/wo_new_features")
sampleFiles <- list.files(path="/scr/k70san/rohit/Lacertidae/splicing/pooled/namesort/counts/wo_new_features",pattern="^wo_new_features_")
...
# LFC < 0 (down)   : 5959, 18% 
# outliers [1]     : 0, 0% 
# low counts [2]   : 1296, 3.9% 
res<-res[order(res$padj),]
head(res)

DESeq2::plotMA(dds,ylim=c(-11,11),main='DESeq2 with Groups and Conditions')
dev.copy(png,'~/Downloads/scripts/wo_new_deseq2_MAplot_mod.png')
dev.off()

diff_genes <- res[which(res$pvalue < 0.05 & res$log2FoldChange > 1),]
write.table(diff_genes, "/scr/k70san/rohit/Lacertidae/splicing/pooled/namesort/counts/wo_new_differentially_expressed_mod.txt", sep="\t")

diff_genes_2 <- res[which(res$pvalue < 0.05 & res$log2FoldChange < 0),]
write.table(diff_genes_2, "/scr/k70san/rohit/Lacertidae/splicing/pooled/namesort/counts/wo_new_lowly_expressed_mod.txt", sep="\t")
...
dev.copy(png,'~/Downloads/scripts/wo_new_deseq2_MAplot_mod_LRT.png')
dev.off()

diff_genes_LRT <- resLRT[which(resLRT$pvalue < 0.05 & resLRT$log2FoldChange > 1),]
write.table(diff_genes_LRT, "/scr/k70san/rohit/Lacertidae/splicing/pooled/namesort/counts/wo_new_differentially_expressed_mod_LRT.txt", sep="\t")

diff_genes_2_LRT <- resLRT[which(resLRT$pvalue < 0.05 & resLRT$log2FoldChange < 0),]
write.table(diff_genes_2_LRT, "/scr/k70san/rohit/Lacertidae/splicing/pooled/namesort/counts/wo_new_lowly_expressed_mod_LRT.txt", sep="\t")
...
head(res_group)
summary(res_group)
# out of 33442 with nonzero total read count
# adjusted p-value < 0.1
# LFC > 0 (up)     : 171, 0.51% 
# LFC < 0 (down)   : 46, 0.14% 
# outliers [1]     : 4557, 14% 
...
dev.copy(png,'~/Downloads/scripts/wo_new_deseq2_MAplot_group.png')
dev.off()

diff_genes_group <- res_group[which(res_group$pvalue < 0.05 & res_group$log2FoldChange > 1),]
write.table(diff_genes_group, "/scr/k70san/rohit/Lacertidae/splicing/pooled/namesort/counts/wo_new_differentially_expressed_group.txt", sep="\t")

diff_genes_group_2 <- res_group[which(res_group$pvalue < 0.05 & res_group$log2FoldChange < 0),]
write.table(diff_genes_group_2, "/scr/k70san/rohit/Lacertidae/splicing/pooled/namesort/counts/wo_new_lowly_expressed_group.txt", sep="\t")
...
dev.copy(png,'~/Downloads/scripts/wo_new_deseq2_MAplot_cond.png')
dev.off()

diff_genes_cond <- res_cond[which(res_cond$pvalue < 0.05 & res_cond$log2FoldChange > 1),]
write.table(diff_genes_cond, "/scr/k70san/rohit/Lacertidae/splicing/pooled/namesort/counts/wo_new_differentially_expressed_cond.txt", sep="\t")

diff_genes_cond_2 <- res_cond[which(res_cond$pvalue < 0.05 & res_cond$log2FoldChange < 0),]
write.table(diff_genes_cond_2, "/scr/k70san/rohit/Lacertidae/splicing/pooled/namesort/counts/wo_new_lowly_expressed_cond.txt", sep="\t")
...
png(file="~/Downloads/scripts/wo_new_deseq2_MAplot_cond_pairwise.png",width=1100,height=800)
par(mfrow=c(5,2))
...
#dev.copy(png,'~/Downloads/scripts/wo_new_deseq2_MAplot_cond_pairwise.png')
dev.off()
