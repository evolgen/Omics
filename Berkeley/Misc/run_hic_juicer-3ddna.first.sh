...
if [ ! -e "./aligned/merged_nodups.txt" ]; then
    ~/local_modules_sw/juicer/1.6.2/CPU/juicer.sh -S dedup -z ${name}_ref.fasta -d $workdir -s HindIII -p ${name}_ref.fasta.fai -t 32 -g ${name} -y ${name}_HindIII.txt 1>LOG 2>>LOG; 
