...
python /global/home/users/rohitkolora/local_modules_sw/salsa/2.2/run_pipeline.py -a ${reference_fasta} -c 1000 -o scaffolds -l ${reference_fasta}.fai -e GATC,GAATC,GAGTC,GACTC,GATTC -i 3 -b ./deduplicated/repli.bed -m yes 1>>LOG 2>>LOG

printf "\tConverting the hic files\n";
convert.sh scaffolds 1>>LOG 2>>LOG
awk -f ~/local_modules_sw/3ddna/utils/generate-assembly-file-from-fasta.awk ./scaffolds/scaffolds_FINAL.fasta >./scaffolds/scaffolds_FINAL.assembly
