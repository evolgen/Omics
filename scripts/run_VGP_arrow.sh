...
for iter in 1; do
	...
	if [ ! -e "pbmap_${reference}.${count}.bam" ]; then
		pbmm2 align --preset SUBREAD ${reference}.${count}.fasta $fastq /clusterfs/genomicdata/pbmap_${reference}.${count}.bam --sort -j 26 -J 6;
        mv /clusterfs/genomicdata/pbmap_${reference}.${count}.bam .;
	...
	if [ ! -e "pbmap_${reference}.pbsort.bam" ]; then
        ...
        samtools sort -T /clusterfs/genomicdata/alitmp -@ 32 -m 1G -o pbmap_${reference}.pbsort.bam pbmap_${reference}.pb.bam
