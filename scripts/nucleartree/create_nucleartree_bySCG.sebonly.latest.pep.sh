...
if [ ! -e "~/RGP/scripts/nucleartree/list_scp_${group}-genes.sebonly.filt.latest.txt" ] ; then
...
if [ ! -e "${working}/sebonly_${group}_PEP.FASTA" ]; then
    count=1
    ...
    while read folder; do
        ...
        printf ">${speciesname}\n" \
            >${working}/sebonly_${group}_per_species/${speciesname}.PEP
       ...
       \ls -d ${working}/sebonly_${group}_per_species/*.PEP |
        xargs cat |
        sed -e 's/>/\n>/g' |
        sed -e '/^$/d' \
            >${working}/sebonly_${group}_PEP.FASTA
    sed -i -e '/^$/d' ${working}/sebonly_${group}_PEP.FASTA;

fi

if [ ! -e "${working}/sebonly_${group}_PEP.maffttrim.FASTA" ]; then
module load trimal mafft raxml gcc muscle java 
printf "\n\tUsing MAFFT for MSA\n";
mafft --auto ${working}/sebonly_${group}_PEP.FASTA >${working}/sebonly_${group}_PEP.mafft.FASTA 

printf "\tUsing TRIMAL for trimming the MSA\n";
trimal -in ${working}/sebonly_${group}_PEP.mafft.FASTA -fasta -out ${working}/sebonly_${group}_PEP.maffttrim.FASTA -gt 1 
fi

if [ ! -e "${working}/RAxML_bestTree.Sebastesonly_${group}_pep" ]; then
    cd ${working}
    printf "\n\tUsing RAxML for tree building\n";
    raxmlHPC-PTHREADS-SSE3 -T 32 -f a -m PROTGAMMAGTR \
        -p 185009 -x 185009 -# 100 \
        -s ${working}/sebonly_${group}_PEP.maffttrim.FASTA \
        -n Sebastesonly_${group}_pep -o Sebastiscus_albofasciatus
fi
