...
filename="~/RGP/scripts/nucleartree/Latest/list_nuclear_samples_${group}.sebonly.uniq.latest.txt"
#if [ ! -f "~/RGP/scripts/nucleartree/Latest/list_nuclear_samples_${group}.sebonly.uniq.latest.txt" ] ; then
...
filename="~/RGP/scripts/nucleartree/Latest/list_scp_${group}-genes.sebonly.filt.latest.txt"
#if [ ! -f "${filename}" ] ; then
#    echo "Missing - $filename"
#    exit 1
#fi
...
mkdir -p ${working}/sebonly_${group}_per_gene;
filename="${working}/sebonly_${group}_per_gene/DONE.txt"
if [ -f "${filename}" ]; then
...
cat ~/RGP/scripts/nucleartree/Latest/list_scp_${group}-genes.sebonly.filt.latest.txt | 
    while read gene; do
        ...
        printf "" >${working}/sebonly_${group}_per_gene/${genename}.FNA;
        printf "" >${working}/sebonly_${group}_per_gene/${genename}.FAA;
        ...
        while read folder; do
            ...
            printf " ${speciesname} ";
            printf ">${speciesname}\n" \
                >>${working}/sebonly_${group}_per_gene/${genename}.FNA
            printf ">${speciesname}\n" \
                >>${working}/sebonly_${group}_per_gene/${genename}.FAA
...
echo "" >${working}/sebonly_${group}_per_gene/DONE.txt;
