...
##sizer=$(cut -f2 ${infile}.fai | sort -u)
##sizer_count=$(cut -f2 ${infile}.fai | sort -u | wc -l)

#cut -f2 ${infile}.fai | sort -u >${infile}.sizes;
...
##if ! (( $sizer % 3 )) ; then
##    printf "\t Size problem : ${infile} = $sizer bp\n\n";
##    exit 1;
##fi
...
grep '^>' $infile | grep -e 'Sebastes_' |
    ...
    while read speciesname; do
        ...
        samtools faidx $infile ${speciesname} |
            ...
            tr "\n" " " | sed -e 's/ //g' -e 's/$/\n/' |
            fold -w 3 |
            sed -e 's/^TAA$/NNN/' -e '$s/^TAG$/NNN/' -e '$s/^TGA$/NNN/' -e '$s/^taa$/NNN/' -e '$s/^tag$/NNN/' -e '$s/^tga$/NNN/' |
            awk 'length($0) == 3' |
            tr "\n" " " | sed -e 's/ //g' -e 's/$/\n/' >>${genefile};
    done          
