...
#use lib './embl_format_fixer/AvrilGffUtils'; #use embl_format_fixer::AvrilGffUtils;
#use lib './embl_format_fixer/AvrilFileUtils';
#use lib './embl_format_fixer/AvrilFastaUtils';
use lib qw(. /global/home/users/rohitkolora/RGP/scripts/annotations/embl_format_fixer);
use AvrilGffUtils;
use AvrilFileUtils;
use AvrilFastaUtils;

#push @INC, '/global/scratch2/rohitkolora/miniconda3/envs/perl/lib/site_perl/5.26.2';
use lib qw(. /global/scratch2/rohitkolora/miniconda3/envs/perl/lib/site_perl/5.26.2);
use Bio::SeqIO;
...
sub run_main_program
{
   ...
   $input_fasta_obj        = Bio::SeqIO->new( '-format' => 'Fasta', '-file' => $input_fasta);
#   AvrilFastaUtils::new(fasta_file => $input_fasta); 

   # CHECK IF ANY OF THE SEQUENCES HAVE INTERNAL STOP CODONS:  
   $protname2internalstops = AvrilFastaUtils::count_internal_stops($input_fasta_obj);
