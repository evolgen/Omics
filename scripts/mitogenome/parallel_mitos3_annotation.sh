...
#\ls -d /global/scratch2/rohitkolora/Rockfish/Genomes/sequencing/illumina/output/*/*/mitogenome/MITOS/sequence.fas-8* | grep -v 'work' |
...
##        tail -n 1 ${working}/MITOS/result.bed >${working}/MITOS/tmp;
##        nonproper=$(awk -F'\t' 'NR==1 {if($3<$2) {print "YES"} else print "NO"}' ${working}/MITOS/tmp);     ### STRAND REVERSAL
##        rm ${working}/MITOS/tmp;

        printf "Running MITOS3\n";
...
#        cat ${working}/ext.bed;
        ...
        if [ "${nonproper}" == "YES" ]; then                ### Negative strand for trn-Phe
            ...
            tac ${working}/ext.bed | 
                ...
                sed -e 's/$/\n/' \
                    >>${working}/${name}.fasta;
            mkdir -p ${working}/MITOS3;
            printf "\tRunning MITOS local version\n";
            runmitos.py -i ${working}/${name}.fasta -o ${working}/MITOS3 \
