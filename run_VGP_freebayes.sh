...
source /global/scratch2/rohitkolora/miniconda3/etc/profile.d/conda.sh
    conda activate gatk                         ### 1st Duplicate Removal
...
source /global/scratch2/rohitkolora/miniconda3/etc/profile.d/conda.sh
    conda activate gatk                                 ### 2nd Duplicate Removal
...
if [ ! -e "${bam2/%.bam/.vcf.gz}.tbi" ]; then
    ...
    freebayes --limit-coverage 200 -f ${consensus}.1.fasta ${bam2} | bcftools view -Ou -e'type="ref"' | bcftools norm -Ob -f ${consensus}.1.fasta -o ${bam2/%.bam/.bcf} --threads 32;
