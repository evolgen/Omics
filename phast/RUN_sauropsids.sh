...
# for file1 in ./new_mafs/Lvir_*.maf; do file2=$(echo $file1 | sed 's/new_mafs/new_mod_mafs/'); /scr/bloodymary/rohit/phast/bin/maf_parse -o MAF -O lacvir1,lacbil1,anoCar2,allMis1,galGal3 $file1 >$file2; done
...
#/scr/bloodymary/rohit/phast/bin/maf_parse -o SS -O lacvir1,lacbil1,anoCar2,allMis1,galGal3 Lacerta_multiple_lacvir1_roast.MAF >Lacerta_multiple_lacvir1_roast.SS; msa_view Lacerta_multiple_lacvir1_roast.SS.maf --in-format MAF --out-format SS > Lacerta_multiple_lacvir1_roast.SS; rm Lacerta_multiple_lacvir1_roast.SS.maf

###/scr/bloodymary/rohit/phast/bin/maf_parse -o MAF -O lacvir1,lacbil1,anoCar2,allMis1,galGal3 Lacerta_multiple_lacvir1_roast.MAF >Lacerta_multiple_lacvir1_roast.MAF2; sed -i -e 2d Lacerta_multiple_lacvir1_roast.MAF2; msa_view Lacerta_multiple_lacvir1_roast.MAF2 --in-format MAF --out-format SS >Lacerta_multiple_lacvir1_roast.SS; rm Lacerta_multiple_lacvir1_roast.MAF2


# sed -e 2d Lacerta_multiple_lacvir1_roast.MAF >Lacerta_multiple_lacvir1_roast.MAF.edit

# phyloFit --msa-format MAF --tree "((anoCar2,(lacvir1,lacbil1)),(galGal3,allMis1))" --out-root Lacerta_init Lacerta_multiple_lacvir1_roast.MAF.edit
# rm Lacerta_multiple_lacvir1_roast.MAF.edit 

printf "Created Lacerta_init model file" 

# msa_view --unordered-ss --aggregate lacvir1,lacbil1,anoCar2,allMis1,galGal3 --in-format SS --out-format SS new_codons_ss/*.codons.ss > Lacerta_all-cons.sites.ss; msa_view --unordered-ss --aggregate lacvir1,lacbil1,anoCar2,allMis1,galGal3 --in-format SS --out-format SS new_sites_ss/*.sites.ss > Lacerta_all-4d.sites.ss

printf "4dss files combined\n"


# phyloFit --tree "((anoCar2,(lacvir1,lacbil1)),(galGal3,allMis1))" --msa-format SS --out-root Lacerta_conserved-codon1 Lacerta_all-cons.sites.ss; phyloFit --tree "((anoCar2,(lacvir1,lacbil1)),(galGal3,allMis1))" --msa-format SS --out-root Lacerta_nonconserved-4d Lacerta_all-4d.sites.ss
...
# ls new_mod_mafs/Lvir_*.maf | parallel -j 5 sh run_new_phylop_maf.sh
...
# ls new_mod_mafs/*.maf | parallel -j 25 sh run_new_phastcons_1.sh; 
printf "RAN     run_new_phastcons_1.sh\n"
# ls new_Chunks/*.ss | parallel -j 25 sh run_new_phastcons_2.sh; 
...
# awk -F'\t' '{print >>"new_count_species."$2}' new_count_species; for file in new_count_species.[0-9]*; do awk -F'\t' '{print $1}' $file >new_phyloboot_$file; done
# for file in new_phyloboot_*; do file2=$(echo $file | sed 's/.*\.//'); str=$(printf "'*"$file"'"); echo phyloBoot --read-mods $str --output-average Lacerta_ave.cons.mod_$file2 ; done; phyloBoot --read-mods '*new_phyloboot_new_count_species.1' --output-average Lacerta_ave.cons.mod_1; phyloBoot --read-mods '*new_phyloboot_new_count_species.2' --output-average Lacerta_ave.cons.mod_2; phyloBoot --read-mods '*new_phyloboot_new_count_species.3' --output-average Lacerta_ave.cons.mod_3; phyloBoot --read-mods '*new_phyloboot_new_count_species.4' --output-average Lacerta_ave.cons.mod_4
# for file in new_count_species.[0-9]*; do awk -F'\t' '{print $1}' $file | sed 's/\.cons\.mod/.noncons.mod/' >new_phyloboot_nc_$file; done; for file in new_phyloboot_nc_*.[1-6]; do file2=$(echo $file | sed 's/.*\.//'); str=$(printf "'*"$file"'"); cmd1=$(echo phyloBoot --read-mods $str --output-average Lacerta_ave.noncons.mod_$file2); eval $cmd1 ; done

 
# ls new_Chunks/*.ss | parallel -j 25 sh run_new_phastcons_4.sh; cat new_Scores/Lvir_*.[1-4] >Lacerta_most-conserved.wig

printf "Made most-conserved from 4 wigs\n"

# cat new_Scores/Lvir_*.1 >Lacerta_most-conserved.1.wig; cat new_Scores/Lvir_*.2 >Lacerta_most-conserved.2.wig; cat new_Scores/Lvir_*.3 >Lacerta_most-conserved.3.wig; cat new_Scores/Lvir_*.4 >Lacerta_most-conserved.4.wig

printf "Made seperate wigs for each number of species:	1-4	\n"

# for file in Lacerta_most-conserved.[1-4].wig; do /scr/bloodymary/rohit/kentUtils-master/bin/linux.x86_64/wigToBigWig -clip $file vir.sizes $file.bigwig; done
printf "Created bigwigs for each 1-4\n"

# /scr/bloodymary/rohit/kentUtils-master/bin/linux.x86_64/bigWigMerge Lacerta_most-conserved.1.wig.bigwig Lacerta_most-conserved.2.wig.bigwig Lacerta_most-conserved.3.wig.bigwig Lacerta_most-conserved.4.wig.bigwig Lacerta_most-conserved.wig.bedgraph

printf "Made bedgraph from 1-4 bigwigs\n"
...
printf "Made most-conserved from 4 species into bedgraph\n"
